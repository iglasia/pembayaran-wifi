...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <p><strong>Mulai Berlangganan:</strong> {{ \Carbon\Carbon::parse($client->subscription_reactivated_at ?? $client->created_at)->translatedFormat('d F Y') }}</p>
    </div>
</div>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr class="text-center">
                        ...
                        <td>{{ indonesian_currency($t['jumlah']) }}</td>
                        <td>
                            @if($t['status'] == 'Lunas')
                        ...
                        <td>
                            {{ $t['tanggal_bayar'] ? \Carbon\Carbon::parse($t['tanggal_bayar'])->translatedFormat('d F Y') : '-' }}
                        </td>
                        <td>
                            @if($t['status'] == 'Belum Lunas')
                                <a href="#" class="btn btn-success btn-sm">Bayar Sekarang</a>
                            @else
                                <a href="{{ route('client.invoice', $t['transaction_id']) }}" class="btn btn-info btn-sm" target="_blank">Lihat Invoice</a>
                            @endif
                        </td>
                    </tr>
                    @empty
