...
class TransactionRepository extends Controller
{
    ...
    public function index()
    {
        ...
        $awalLangganan = Carbon::parse($client->created_at);
        ...
        // Cek status bayar tiap bulan
        $tagihan = [];
        foreach ($daftarTagihan as $dt) {
            $sudahBayar = $transaksi->where('month', $dt['bulan'])->where('year', $dt['tahun'])->first();
            $tagihan[] = [
                'bulan' => $dt['bulan'],
                'tahun' => $dt['tahun'],
                'status' => $sudahBayar ? 'Lunas' : 'Belum Lunas',
                'jumlah' => $sudahBayar ? $sudahBayar->amount : $client->internet_package->price,
                'tanggal_bayar' => $sudahBayar ? $sudahBayar->created_at : null,
            ];
        }

        return view('client.billing', compact('tagihan', 'client'));
    }
}
